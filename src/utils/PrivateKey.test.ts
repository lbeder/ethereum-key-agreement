import { PrivateKey, COMPRESSED_PRIVATE_KEY_LENGTH } from './PrivateKey';
...
describe('PrivateKey', () => {
  ...
  describe('isValid', () => {
    [
      ...
      '0x42fc0028b60a5052de140b7b5bad25f01041ec306b1e59714963f38661ca1e7bab',
      '0x42fc0028b60a5052de140b7b5bad25f01041ec306b1e59714963f38661ca1e',
      Buffer.from('9c2044b4c7ee77b0d9a6105f0d50c0f397efd5317d47c21b8fab8fb2060dcd', 'hex'),
      Buffer.from('611bedf21d9069610df3f70553783f4e04a8ef1325e15123b1d435cb5bb484bdaa', 'hex')
    ].forEach((key) => {
      it(`should invalidate key ${key.toString('hex')}`, async () => {
        expect(PrivateKey.isValid(key)).toBeFalsy();
      });
    });
    ...
    [
      ...
      it(`should validate key ${key.toString('hex')}`, async () => {
        expect(PrivateKey.isValid(key)).toBeTruthy();
      });
    });
  });
